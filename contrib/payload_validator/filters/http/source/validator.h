...
public:
  ...
  QueryParamValidator(const std::string param_name)
      : ParamValidatorBase(param_name), required_(true) {}
...
public:
  ...
  FixedPathSegmentValidator(const absl::string_view segment, uint32_t number)
      : segment_(segment), segment_number_(number) {}
  uint32_t segmentNumber() const { return segment_number_; }

  bool validate(const absl::string_view segment) const {
    return segment_ == Http::LowerCaseString(segment);
  }
...
public:
  ...
  TemplatedPathSegmentValidator(const std::string segment_name, const uint32_t number)
      : ParamValidatorBase(segment_name), segment_number_(number) {}
...
using AllowedPaths =
    absl::flat_hash_map<uint32_t, std::vector<std::unique_ptr<PathTemplateValidator>>>;
