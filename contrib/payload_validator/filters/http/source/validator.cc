...
ParamValidatorBase::validate(absl::string_view param_value) {
  ...
  std::string to_test = "{\"" + std::string(param_name_) + "\":" + value_decorator_ +
                        std::string(param_value) + value_decorator_ + "}";
  std::cerr << to_test << "\n";
  ...
  try {
    ...
    return std::make_pair<bool, absl::optional<std::string>>(
        false, fmt::format("Parameter '{}' contains unexpected characters or is of wrong type",
                           param_name_));
  }
  ...
  try {
    ...
    return std::make_pair<bool, absl::optional<std::string>>(
        false, fmt::format("Parameter {} does not match the schema: {}", param_name_, e.what()));
  }
...
std::pair<bool, absl::optional<std::string>> validateParams(
  ...
  for (auto param : param_tokens) {
    ...
    if (name_end != absl::string_view::npos) {
      ...
      if (!result.first) {
        return std::make_pair<bool, absl::optional<std::string>>(
            false, absl::optional<std::string>(fmt::format("Validation of parameter {} failed: {}",
                                                           param_name, result.second.value())));
      }
...
checkPath(const PathTemplateValidator& path_template,
          const std::vector<absl::string_view>& path_segments) {

  ASSERT(path_segments.size() ==
         (path_template.fixed_segments_.size() + path_template.templated_segments_.size()));
  ...
  for (auto& templated_segment : path_template.templated_segments_) {
    ...
    if (!result.first) {
      return std::make_pair<PathValidationResult, absl::optional<std::string>>(
          PathValidationResult::MATCHED_WITH_ERRORS, std::move(result.second));
    }
  }

  return std::make_pair<PathValidationResult, absl::optional<std::string>>(
      PathValidationResult::MATCHED, absl::nullopt);
}
...
std::pair<bool, absl::optional<std::string>> JSONBodyValidator::validate(absl::string_view body) {
  json rec_buf;
  try {
    ...
    return std::make_pair<bool, absl::optional<std::string>>(
        false, fmt::format("Payload is not a valid JSON document: {}", e.what()));
  }
  ...
  try {
    ...
    return std::make_pair<bool, absl::optional<std::string>>(
        false, fmt::format("Payload does not match the schema: {}", e.what()));
  }
