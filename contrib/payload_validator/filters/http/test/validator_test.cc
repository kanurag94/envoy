...
TEST(QueryParamValidator, ValidateQueryParams) {
  ...
  std::unique_ptr<QueryParamValidator> param_validator =
      std::make_unique<QueryParamValidator>("param1");
...
TEST(FixedPathSegmentValidator, AllFixedPathsComparison) {
  ...
  ASSERT_THAT(checkPath(path_template, {"segment1", "segment2"}).first,
              PathValidationResult::MATCHED);
  ASSERT_THAT(checkPath(path_template, {"segment2", "segment1"}).first,
              PathValidationResult::NOT_MATCHED);
  ASSERT_THAT(checkPath(path_template, {"segment1", "segment1"}).first,
              PathValidationResult::NOT_MATCHED);
  ASSERT_THAT(checkPath(path_template, {"segment2", "segment2"}).first,
              PathValidationResult::NOT_MATCHED);
  ASSERT_THAT(checkPath(path_template, {"segment1", "segment3"}).first,
              PathValidationResult::NOT_MATCHED);
  ASSERT_THAT(checkPath(path_template, {"segment3", "segment2"}).first,
              PathValidationResult::NOT_MATCHED);
...
TEST(TemplatedPathSegmentValidation, AllTemplatedPathsComparison) {
  ...
  PathTemplateValidator path_template;
  path_template.templated_segments_.push_back(
      std::make_unique<TemplatedPathSegmentValidator>("segment1", 0));
  ...
  ASSERT_THAT(checkPath(path_template, {"\"segment1\""}).first,
  ...
  path_template.templated_segments_.push_back(
      std::make_unique<TemplatedPathSegmentValidator>("segment2", 0));
  // segment2 should be integer.
  ASSERT_TRUE(path_template.templated_segments_.back()->initialize("{\"type\": \"integer\"}"));
  path_template.templated_segments_.push_back(
      std::make_unique<TemplatedPathSegmentValidator>("segment3", 1));
  ...
  ASSERT_THAT(checkPath(path_template, {"part1", "part2"}).first,
              PathValidationResult::MATCHED_WITH_ERRORS);
}
...
TEST(TemplatedPathSegmentValidation, MixedPathsComparison) {
  ...
  path_template.templated_segments_.push_back(
      std::make_unique<TemplatedPathSegmentValidator>("segment4", 3));
  // segment3 should be string.
  ASSERT_TRUE(path_template.templated_segments_.back()->initialize("{\"type\": \"integer\"}"));
  ...
  ASSERT_THAT(checkPath(path_template, {"segment1", "segment2", "segment3", "123"}).first,
              PathValidationResult::MATCHED);
  ASSERT_THAT(checkPath(path_template, {"segment1", "segment2", "segment3", "segment4"}).first,
              PathValidationResult::MATCHED_WITH_ERRORS);
  ASSERT_THAT(checkPath(path_template, {"123", "segment2", "segment3", "segment4"}).first,
              PathValidationResult::MATCHED_WITH_ERRORS);
  ASSERT_THAT(checkPath(path_template, {"segment1", "segment2", "segment33", "123"}).first,
              PathValidationResult::NOT_MATCHED);
}
